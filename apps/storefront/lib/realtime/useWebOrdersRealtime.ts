...
/**
 * The live order-status page's subscription — BUILD-PLAN.md §16;
 * docs/runfiles/M14-storefront.md §2/§3.
 *
 * A straight copy of `apps/pos/lib/realtime/useFloorRealtime.ts`'s own
 * `EventSource` workaround — see that file's doc comment for why a raw
 * `EventSource` against `/api/realtime` is used here rather than
 * `@natech/realtime/client`'s `useRealtime` hook (no `RealtimeProvider` is
 * mounted in any of the three apps yet). Carried forward there and here
 * alike.
 */
